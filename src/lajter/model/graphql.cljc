...
(defn parse-schema-str
  "Parses a graphql schema string, omitting nil values
  and transforms names and types to symbols."
  [schema-str]
  (let [value-fn (fn self [k v]
                   (let [#?@(:clj  [omit self]
                             :cljs [omit js/undefined
                                    coll? #(or (coll? %) (array? %))])]
                     (cond
                       (nil? v) omit
                       (and (coll? v) (empty? v)) omit
                       (#{"name" "kind"} (name k)) (symbol v)
                       :else v)))]
    #?(:clj
       (json/read-str schema-str
                      :key-fn keyword
                      :value-fn value-fn)
       :cljs
       (js->clj (js/JSON.parse schema-str value-fn)
...
(defn schema->model
  ...
  * Input fields with names and type information."
  ...
  ([parsed-schema]
   (into []
         (schema->model)
         (get-in parsed-schema [:data :__schema :types]))))
...
(def plugin:graphql-input-fields
  {:db/schema
   {:gql.field.input/field {:db/valueType :db.type/ref}}
   :rule/clauses
   '[[(input-fields ?field ?input)
      [?input :gql.field.input/field ?field]]
     [(input-less ?field)
      (not [_ :gql.field.input/field ?field])]]
   :pipeline/fn
   (fn [{:keys [db]}]
     (mapcat
       (fn [node]
         (let [->field-node #(field-input->field-node db node %)]
           ...
           ;; each of them.
           (cons (update node :model.node/meta dissoc :gql.field/inputs)
                 (map ->field-node (get-in node [:model.node/meta
                                                 :gql.field/inputs])))))))})
...
(defn pattern->query
  "Turns a pull pattern (conforming to ::model/model) and returns
  a graphql query. Needs to start with [Query [(<field> [selector])+]]"
  [pattern]
  {:pre [(== 2 (count pattern))
         (= 'Query (first pattern))]}
  (str "query"
